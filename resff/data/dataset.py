...
class Dataset(abc.ABC, torch.utils.data.Dataset):
    ...
    def regenerate_impropers(self, improper_def="smirnoff"):
        ...
        """
        from resff.graphs.utils.regenerate_impropers import (
...
class GraphDataset(Dataset):
    ...
    def __init__(self, graphs=[], first=None):
        ...
        if all(
            ...
            if first is None or first == -1:
                graphs = [resff.Graph(graph) for graph in graphs]

            else:
                graphs = [resff.Graph(graph) for graph in graphs[:first]]
    ...
    def batch(graphs):
        ...
        if all(isinstance(graph, resff.graphs.graph.Graph) for graph in graphs):
    ...
    def load(cls, path):
        ...
        graphs = []
        for _path in paths:
            graphs.append(resff.Graph.load(path + "/" + _path))
