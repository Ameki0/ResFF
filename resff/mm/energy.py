...
# =============================================================================
def get_bondfrommm(g, suffix=""):
    return {
        "u%s"
        % suffix: resff.data.md.calculate_bond_energy(g)
    }
...
def apply_bond(nodes, suffix=""):
    ...
    return {
        "u%s"
        % suffix: resff.mm.bond.harmonic_bond(
            x=nodes.data["x"],
...
def apply_angle(nodes, suffix=""):
    ...
    return {
        "u%s"
        % suffix: resff.mm.angle.harmonic_angle(
            x=nodes.data["x"],
            k=nodes.data["k%s" % suffix],
            eq=nodes.data["eq%s" % suffix],
        )
...
def apply_bond_ii(nodes, suffix=""):
    return {
        "u_bond_high%s"
        % suffix: resff.mm.bond.bond_high(
            u_bond=nodes.data["u"],
            k3=nodes.data["k3"],
            k4=nodes.data["k4"],
        )
...
def apply_torsion_ii(nodes, suffix=""):
    ...
    return {
        ...
        % suffix: resff.mm.torsion.angle_angle_torsion(
            u_angle_left=nodes.data["u_angle_left"],
            u_angle_right=nodes.data["u_angle_right"],
            u_torsion=nodes.data["u"],
        ...
        "u_bond_torsion%s"
        % suffix: resff.mm.torsion.bond_torsion(
...
def apply_torsion(nodes, suffix=""):
    ...
    if (
        ...
        return {
            "u%s"
            % suffix: resff.mm.torsion.periodic_torsion(
                x=nodes.data["x"],
                k=nodes.data["k%s" % suffix],
...
def apply_bond_gaussian(nodes, suffix=""):
    ...
    return {
        "u%s"
        % suffix: resff.mm.bond.gaussian_bond(
            x=nodes.data["x"],
...
def apply_bond_linear_mixture(nodes, suffix="", phases=[0.0, 1.0]):
    ...
    return {
        "u%s"
        % suffix: resff.mm.bond.linear_mixture_bond(
            x=nodes.data["x"],
            coefficients=nodes.data["coefficients%s" % suffix],
            phases=phases,
        )
...
def apply_angle_linear_mixture(nodes, suffix="", phases=[0.0, 1.0]):
    ...
    return {
        "u%s"
        % suffix: resff.mm.angle.linear_mixture_angle(
            x=nodes.data["x"],
            coefficients=nodes.data["coefficients%s" % suffix],
            phases=phases,
        )
...
def energy_in_graph(
    ...
    if "n2" in terms:
        ...
        #g.apply_nodes(
        #        lambda node: get_bondfrommm(g, suffix=suffix),
        #        ntype="n2",
        #    )
        ...
        if "coefficients%s" % suffix in g.nodes["n2"].data:
            g.apply_nodes(
                ...
                apply_bond_linear_mixture(
                    node, suffix=suffix, phases=[0.7935, 3.174]  
                ),
                ntype="n2",
            )
        else:
