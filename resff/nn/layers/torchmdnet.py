...
from resff.nn.layers.torchmdnet_utils import (
    NeighborEmbedding,
    CosineCutoff,
    Distance,
...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dim_transform = nn.Linear(128, hidden_channels) #to change the input dimension
    ...
    def forward(self,
        ...
        n_conf=graph.nodes['n1'].data['xyz'].shape[1]
        ...
        y = y.view(-1, n_conf, 1)
        y = y.squeeze(dim=2)
        graph.nodes['g'].data['u_residual'] = y

        del y, sum, x, batch

        return graph.nodes["n1"].data["h"]
