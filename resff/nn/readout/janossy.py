...
class JanossyPooling(torch.nn.Module):
    ...
    def __init__(
        ...
        for level in self.levels:
            ...
            setattr(
                self,
                "sequential_%s" % level,
                resff.nn.sequential._Sequential(
        ...
        # atom level
        self.sequential_1 = resff.nn.sequential._Sequential(
    ...
    def forward(self, g, coord_feat=None, batch=None):
        """Forward pass.
        ...
        for big_idx in self.levels:
            ...
            if g.number_of_nodes("n%s" % big_idx) == 0:
                continue
            import logging
            #logging.debug(print(f'f_out_{big_idx}'))
            g.apply_nodes(
                func=lambda nodes: {
                    feature: getattr(
        ...
        g.apply_nodes(
            func=lambda nodes: {
                feature: getattr(self, "f_out_1_to_%s" % feature)(
                    self.sequential_1(_g=None, x=nodes.data["h"],coord_feat=None,batch=None, g=None)
                )
                    ...
                    feature: getattr(self, f"f_out_{big_idx}_to_{feature}")(
                        torch.sum(
                            torch.stack(
                                [
                                    inner_net(
                                        _g=None, x=stack_permuts(nodes, p), coord_feat=None,batch=None,g=None
                                    )
                                    for p in permuts
                                ],
                                dim=0,
...
class LinearMixtureToOriginal(torch.nn.Module):
    def forward(self, g):
        ...
        (
            g.nodes["n2"].data["k"],
            g.nodes["n2"].data["eq"],
        ) = resff.mm.functional.linear_mixture_to_original(
