...
from residual.models import output_modules
from residual.models.wrappers import AtomFilter
from residual.models.utils import dtype_mapping
from residual import priors
...
def create_model(args, prior_model=None, mean=None, std=None):
    ...
    shared_args = dict(
        ...
        max_z=args["max_z"],
        check_errors=False,#bool(args["check_errors"]),
    ...
    if args["model"] == "graph-network":
        from residual.models.torchmd_gn import TorchMD_GN
        ...
        from residual.models.torchmd_t import TorchMD_T
        ...
        from residual.models.torchmd_et import TorchMD_ET
        ...
        from residual.models.tensornet import TensorNet
    ...
    elif args["model"] == "vector-tensornet":
        from residual.models.tensornet import TensorNet
...
def load_model(filepath, args=None, device="cpu", mean=None, std=None, prior_model=None, **kwargs):
    ...
    if args["pretrained_model"] is not None:
        model = create_model(args, prior_model=prior_model)
    else:
...
class TorchMD_Net(nn.Module):
    ...
    def forward(
        ...
        Returns:
            Tuple[Tensor, Optional[Tensor], Tensor]: The output of the model and the derivative of the output with respect to the positions if derivative is True, None otherwise.
        """
        assert z.dim() == 2 
        #batch = torch.zeros_like(z) if batch is None else batch
        batch = torch.zeros((z.size(0), 1), dtype=torch.long, device=z.device).squeeze(dim=1) if batch is None else batch
        if self.derivative:
            pos.requires_grad_(True)
        # run the potentially wrapped representation model
        y = self.representation_model(
