...
from residual.models.utils import (
    NeighborEmbedding,
    CosineCutoff,
    OptimizedDistance,
    rbf_class_mapping,
...
from residual.utils import deprecated_class
...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        self.f_in = torch.nn.Sequential(
            torch.nn.Linear(114, 128), torch.nn.Tanh()
        )
        ...
        self.neighbor_embedding = (
            NeighborEmbedding(
                hidden_channels, num_rbf, cutoff_lower, cutoff_upper, self.max_atom_type, dtype
            )
            if neighbor_embedding
            else None
        )
        self.dim_transform = nn.Linear(128, hidden_channels)#
    ...
    def forward(
        ...
        z = self.f_in(input)
        z = self.dim_transform(z)
        n_conf=1
        #x = torch.cat([z] * n_conf, dim=0)
        x = z.repeat(n_conf, 1)
        #pos = pos.to(x.device, non_blocking=True).transpose(0, 1).reshape(-1, pos.shape[-1])
        counts = torch.unique(batch, return_counts=True)[1]
        _bacth = torch.repeat_interleave(torch.arange(counts.shape[0] * n_conf, device=x.device), counts)
        batch = _bacth.to(x.device, non_blocking=True)
        #pos = pos.to(x.device, non_blocking=True).transpose(0, 1).reshape(-1, pos.shape[-1])
        ...
        if self.neighbor_embedding is not None:
            x = self.neighbor_embedding(x, x, edge_index, edge_weight, edge_attr)
        ...
        for lidx, attn in enumerate(self.attention_layers):
            ...
            x = x + dx 
            vec = vec + dvec
            # if self.layernorm_in_att:
            #     x = self.x_norm(x)
            #     vec=self.vec_norms[lidx](vec)
        del vec , edge_index, edge_weight, edge_vec, edge_attr, pos, _bacth, dx, dvec, counts
        x = self.out_norm(x)
        # if self.layernorm_on_vec:
        #     vec = self.out_norm_vec(vec)
        x = self.energy_head(x) 
        sum = scatter(x, batch, dim=0, reduce='add')

      
        y = sum.view(-1, n_conf, 1)
        # 移除额外的维度，将形状调整为 (n/50, 50)
        y = y.squeeze(dim=2)
