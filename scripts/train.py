...
def _load_datasets(datasets, input_prefix):
    """
    Load unique molecules (nonisomeric smile) and split into
...
def run(kwargs):
    ...
    ds_tr, ds_vl , ds_te = _load_datasets(datasets, input_prefix)
    ...
    ds_vl_augment = _augment_conformations(ds_vl, n_max_confs)
    logging.debug(f"# Training size after augment: {len(ds_tr_augment)}")
    logging.debug(f"# Validation size after augment: {len(ds_vl_augment)}")
...
@click.option("-s", "--stage", default="stage_2", type=click.Choice(["stage_1", "stage_2"]), help="stage of training")
